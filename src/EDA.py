...
def plot_prices_together(df: pd.DataFrame, outpng: Path, limit: int | None = None, title: str = "Evolution des prix"):
    ...
    plt.legend(title="Legende", ncols=2, fontsize=8)
    plt.tight_layout()
...
def plot_qq_returns(df: pd.DataFrame, outdir: Path, ret_suffix: str = "_ret", limit: int | None = 6):
    ...
    for c in ret_cols:
        ...
        fig = sm_api.ProbPlot(s).qqplot(line="s")
...
def plot_acf_squared_returns(df: pd.DataFrame, outdir: Path, ret_suffix: str = "_ret", lags: int = 60, limit: int | None = 6):
    """
    ACF des rendements au carré (r²) pour détecter la mémoire de volatilité.
    """
    if sm is None:
...
def plot_acf_pacf_prices(df: pd.DataFrame,
                         outdir: Path,
                         price_suffix: str = "_Close",
                         lags: int = 60,
                         use_log: bool = True,
                         diff_order: int = 1,
                         limit: int | None = 6):
    """
    ACF/PACF pour les PRIX (par défaut sur Δ^d log(P)).
    """
    if sm is None:
        print("statsmodels non dispo => ACF/PACF prix ignorés.")
        return

    price_cols = [c for c in df.columns if c.endswith(price_suffix)]
    if not price_cols:
        print(f"Aucune colonne *{price_suffix} trouvée pour ACF/PACF prix.")
        return
    if limit:
        price_cols = price_cols[:limit]
    ...
    for c in price_cols:
        ...
        if use_log:
            s = np.log(s_raw).replace([np.inf, -np.inf], np.nan).dropna()
        else:
            s = s_raw.copy()
        ...
        label = c
        if use_log and diff_order > 0:
            label += " — Δ^%d log(P)" % diff_order
        elif use_log and diff_order == 0:
            label += " — log(P)"
        elif not use_log and diff_order > 0:
            label += " — Δ^%d P" % diff_order
        else:
            label += " — niveau"

        plt.figure(figsize=(8, 4))
        plot_acf(s, lags=lags, zero=False)
        plt.title(f"ACF — {label}")
        plt.tight_layout()
        plt.savefig(outdir / f"acf_price_{c}.png", dpi=150)
        plt.close()

        plt.figure(figsize=(8, 4))
        plot_pacf(s, lags=lags, method="ywm", zero=False)
        plt.title(f"PACF — {label}")
        plt.tight_layout()
        plt.savefig(outdir / f"pacf_price_{c}.png", dpi=150)
        plt.close()
...
def plot_weekday_seasonality(df: pd.DataFrame, outpng: Path, ret_suffix: str = "_ret"):
    ...
    plt.xlabel("Jour"); plt.ylabel("Moyenne du rendement")
    plt.tight_layout();
...
def conclude_tests(df: pd.DataFrame, outdir: Path, ret_suffix: str = "_ret",
    ...
    """
    Sauvegarde un tableau unique 'test_conclusions.csv' pour chaque série *_ret.
    """
    outdir.mkdir(parents=True, exist_ok=True)
    rows = []
    ...
    for c in ret_cols:
        ...
        try:
            ...
            row["Kurtosis_excess"] = float(kurt - 3.0)
            rows.append(row)
        except Exception:
...
def decompose_with_cycle(df: pd.DataFrame,
                         col: str,
                         outdir: Path,
                         period: int,
                         use_log: bool = False,
                         ...
                         title: str | None = None):
    """
    Décomposition en 4 composantes : Trend / Seasonality / Cyclicity / Irregularity.
    """
    from statsmodels.tsa.seasonal import STL
    try:
        from statsmodels.tsa.filters.cf_filter import cffilter as _cffilter
        has_cff = True
    except Exception:
        has_cff = False
    try:
    ...
    if use_log:
        s = np.log(s.replace({0: np.nan})).replace([np.inf, -np.inf], np.nan).dropna()
    for _ in range(max(0, diff_order)):
        s = s.diff().dropna()

    if len(s) < 3 * max(2, period):
        print(f"[WARN] '{col}' trop court pour period={period}. len={len(s)}")
        return

    stl = STL(s, period=period, robust=robust, seasonal=seasonal_len)
    stl_res = stl.fit()
    observed = s
    trend = stl_res.trend
    seasonal = stl_res.seasonal
    deseasonal = observed - seasonal

    if has_cff and len(deseasonal) >= cycle_high + 5:
        cycle_cf, trend_cf = _cffilter(deseasonal, low=cycle_low, high=cycle_high, drift=False)
        cycle = cycle_cf
    elif has_hp:
        lam = 129600 * (252 / 12) ** 2
        trend_hp, cycle_hp = _hpfilter(deseasonal, lamb=lam)
        cycle = cycle_hp
    else:
        cyc_ma = deseasonal.rolling(window=min(max(50, period*5), len(deseasonal)//3), min_periods=1, center=True).mean()
        cycle = deseasonal - cyc_ma

    irregular = deseasonal - trend.reindex_like(deseasonal).fillna(method="ffill").fillna(method="bfill") - cycle
    irregular = irregular.reindex_like(observed)

    tag = fname_prefix or f"{col}_log{int(use_log)}_d{diff_order}_per{period}_cyc{cycle_low}-{cycle_high}"
    comp = pd.DataFrame({
        "observed": observed,
        "trend": trend.reindex_like(observed),
        "seasonal": seasonal.reindex_like(observed),
        "cycle": cycle.reindex_like(observed),
        "irregular": irregular
    })
    comp_path = outdir / f"decomp_components_{tag}.csv"
    comp.to_csv(comp_path)
    print(f"[OK] Composantes -> {comp_path}")

    fig, axes = plt.subplots(2, 2, figsize=(10, 7), sharex=True)
    axes = axes.ravel()
    observed.plot(ax=axes[0]); axes[0].set_title((title or col) + " — Trend"); axes[0].plot(trend, linewidth=1.2)
    (seasonal).plot(ax=axes[1]); axes[1].set_title(f"Seasonality (period={period})")
    (cycle).plot(ax=axes[2]); axes[2].set_title(f"Cyclicity ({cycle_low}–{cycle_high} jours)")
    (irregular).plot(ax=axes[3]); axes[3].axhline(0, lw=0.8, alpha=0.6); axes[3].set_title("Irregularity (randomness)")
    for ax in axes: ax.grid(True, alpha=0.2)
    plt.tight_layout()
    fig_path = outdir / f"decomp_plot_{tag}.png"
...
if __name__ == "__main__":
    ...
    plot_prices_together(data_df, figs / "prices_together.png", limit=None)
    ...
    # 7-bis) ACF/PACF des PRIX
    plot_acf_pacf_prices(
        data_df,
        figs / "acf_pacf_prices",
        lags=5,
        use_log=True,
        diff_order=1,
        limit=6
    ...
    # Décompositions — indentation corrigée
    decomp_dir = figs / "decompositions_nvda"
    ensure_dir(decomp_dir)

    # --- PRIX NVDA ---
    decompose_with_cycle(
        data_df, col="asset_Close", outdir=decomp_dir,
        period=252, use_log=True, diff_order=0,
        cycle_low=20, cycle_high=252,
        fname_prefix="NVDA_price_logP_annual", title="NVDA — log(P)"
    )
    decompose_with_cycle(
        data_df, col="asset_Close", outdir=decomp_dir,
        period=5, use_log=True, diff_order=1,
        cycle_low=20, cycle_high=252,
        fname_prefix="NVDA_price_dlogP_weekly", title="NVDA — Δlog(P)"
    )

    # --- RENDEMENTS NVDA ---
    decompose_with_cycle(
        data_df, col="asset_ret", outdir=decomp_dir,
        period=5, use_log=False, diff_order=0,
        cycle_low=20, cycle_high=252,
        fname_prefix="NVDA_ret_weekly", title="NVDA — Returns"
    )
    decompose_with_cycle(
        data_df, col="asset_ret", outdir=decomp_dir,
        period=21, use_log=False, diff_order=0,
        cycle_low=20, cycle_high=252,
        fname_prefix="NVDA_ret_monthly", title="NVDA — Returns (~monthly seasonality)"
    )
